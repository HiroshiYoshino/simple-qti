  ...
  var pci = {
    ...
    _render: function () {
      ...
      style.textContent =[
        ...
        ".dice-choice{border:1px solid #cfd7e3;border-radius:8px;padding:8px;background:#fff;cursor:pointer;}",
        ...
        /* 展開図のアニメーション用（ネスト構造） */
        ".dice-net-anim{position:absolute;left:50%;top:50%;width:230px;height:190px;transform:translate(-50%,-50%);perspective:800px;}",
        ".dice-3d-space{position:absolute;width:100%;height:100%;transform-style:preserve-3d;transition:transform 1.0s ease;}",
        ".dice-3d-space.folding{transform:translateZ(-40px) rotateX(-24deg) rotateY(34deg);}",
        ".dice-node{position:absolute;width:44px;height:44px;transform-style:preserve-3d;transition:transform 0.6s ease;}",
        ".node-right{left:44px;top:0;transform-origin:0% 50%;}",
        ".node-left{left:-44px;top:0;transform-origin:100% 50%;}",
        ".node-bottom{left:0;top:44px;transform-origin:50% 0%;}",
        ".node-top{left:0;top:-44px;transform-origin:50% 100%;}",
        ".dice-face-inner{position:absolute;left:0;top:0;width:100%;height:100%;box-sizing:border-box;border:2px solid #365173;border-radius:6px;background:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;color:#28415f;backface-visibility:hidden;transition:background-color 0.4s ease, border-color 0.4s ease;}",
        ".face-back-side{transform:rotateY(180deg) translateZ(0.5px);background:#e4ebf3;border-color:#b5c4d6;}",
        ".dice-face-inner.conflict{background:#ffe7ea;border-color:#b44b5f;color:#8e2a3d;box-shadow:0 0 0 2px rgba(180,75,95,0.2);animation:conflictPulse 0.6s ease-in-out 2;}",
        /* 組み上がった後の完成品 */
        ...
        /* エラー表示 */
        ".dice-fail{position:absolute;left:50%;top:50%;width:220px;height:130px;transform:translate(-50%,-50%);opacity:0;transition:opacity 0.4s ease;}",
        ".dice-fail.show{opacity:1;}",
        ".dice-fail-note{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);padding:8px 10px;border-radius:8px;border:1px solid #c26a7a;background:#fff0f3;color:#7a3040;font-size:12px;font-weight:700;text-align:center;width:100%;box-sizing:border-box;}",
        ...
        /* アニメーションキーフレーム */
        "@keyframes diceSpin{0%{transform:translate(-50%,-50%) rotateX(-18deg) rotateY(20deg) scale(0.6);}100%{transform:translate(-50%,-50%) rotateX(-24deg) rotateY(394deg) scale(1);}}",
        "@keyframes conflictPulse{0%{transform:scale(1);}50%{transform:scale(1.1);}100%{transform:scale(1);}}"
      ].join("");
    ...
    _startSimulation: function (optionId, animate) {
      ...
      stage.innerHTML = "";
      var scene = this._buildNetAnimation(option);
      ...
      if (!animate) {
        ...
        this._timers.push(setTimeout(function () {
          if (foldable) {
            // 折り畳みが完了したら展開図をフェードアウトして立方体に切り替え
            scene.base.style.transition = "opacity 0.4s ease";
            scene.base.style.opacity = "0";
            cube.classList.add("show");
          } else {
            fail.classList.add("show");
    ...
    _buildTree: function (cells) {
      ...
      var bestNeighbors = -1;
      var dirs =[
        { dx: 1, dy: 0, dir: "right", rot: "rotateY(90deg)" },
        { dx: -1, dy: 0, dir: "left", rot: "rotateY(-90deg)" },
        { dx: 0, dy: 1, dir: "bottom", rot: "rotateX(-90deg)" },
        { dx: 0, dy: -1, dir: "top", rot: "rotateX(90deg)" }
      ];

      for (i = 0; i < cells.length; i += 1) {
        var nCount = 0;
        for (var d = 0; d < dirs.length; d += 1) {
          if (map[(cells[i].x + dirs[d].dx) + "," + (cells[i].y + dirs[d].dy)]) {
            nCount += 1;
          }
        }
      ...
      var rootNode = { cell: pivot, children: [], dir: "", rot: "", depth: 0, index: indexMap[k(pivot)] };
      var seen = {};
      seen[k(pivot)] = true;
      var queue =[rootNode];
    ...
    /* ツリー構造からネストされたDOMを生成する */
    _createNodeDom: function (node, nodesMap) {
      var wrapper = document.createElement("div");
      wrapper.className = "dice-node " + (node.dir ? "node-" + node.dir : "node-root");

      var inner = document.createElement("div");
      inner.className = "dice-face-inner";
      inner.textContent = String(node.index + 1);

      var back = document.createElement("div");
      back.className = "dice-face-inner face-back-side";

      wrapper.appendChild(inner);
      wrapper.appendChild(back);

      nodesMap[k(node.cell)] = {
        wrapper: wrapper,
        inner: inner,
        node: node
      };
      ...
      for (var i = 0; i < node.children.length; i += 1) {
        wrapper.appendChild(this._createNodeDom(node.children[i], nodesMap));
      }
      return wrapper;
    },
    ...
    _buildNetAnimation: function (option) {
      ...
      var tree = this._buildTree(option.cells);
      var nodesMap = {};
      var rootDom = this._createNodeDom(tree, nodesMap);

      var offsetX = Math.floor((230 - w) / 2) + (tree.cell.x - minX) * size;
      var offsetY = Math.floor((190 - h) / 2) + (tree.cell.y - minY) * size;

      rootDom.style.left = String(offsetX) + "px";
      rootDom.style.top = String(offsetY) + "px";

      space.appendChild(rootDom);
    ...
    _playFoldAnimation: function (scene, analysis, animate) {
      var baseDelay = animate ? 100 : 0;
      var stepDelay = animate ? 300 : 0;
      var maxDepth = 0;
      var key;

      // 立体感が伝わるように全体の空間を斜めに向ける
      if (animate) {
        this._timers.push(setTimeout(function () {
          scene.space.classList.add("folding");
        }, 30));
      } else {
        scene.space.classList.add("folding");
      }
      ...
      var applyFold = function () {
        for (key in scene.nodes) {
          ...
          var nodeData = scene.nodes[key];
          if (nodeData.node.depth > maxDepth) {
            maxDepth = nodeData.node.depth;
          }
          if (nodeData.node.dir) {
            var delay = baseDelay + (nodeData.node.depth - 1) * stepDelay;
            nodeData.wrapper.style.transitionDelay = String(delay) + "ms";
            nodeData.wrapper.style.transform = nodeData.node.rot;
          }
        }
      };

      if (animate) {
        this._timers.push(setTimeout(applyFold, 50));
      } else {
        applyFold();
      }

      // 組み立て不可能な場合、衝突した面を赤く光らせる
      if (!analysis.foldable) {
        var conflictKeys = {};
        for (var i = 0; i < analysis.conflictPairs.length; i += 1) {
        ...
        var markConflict = function () {
          for (key in conflictKeys) {
            if (Object.prototype.hasOwnProperty.call(conflictKeys, key) && scene.nodes[key]) {
              scene.nodes[key].inner.classList.add("conflict");
            }
          }
        };
        if (animate) {
          this._timers.push(setTimeout(markConflict, baseDelay + maxDepth * stepDelay + 500));
        } else {
          markConflict();
        }
      }

      return baseDelay + maxDepth * stepDelay + (animate ? 1200 : 0);
    },
    ...
    _analyzeNet: function (cells) {
      ...
      while (queue.length) {
        var c = queue.shift();
        for (i = 0; i < dirs0.length; i += 1) {
          var nk = (c.x + dirs0[i].dx) + "," + (c.y + dirs0[i].dy);
          if (map[nk] && !seen[nk]) {
            seen[nk] = true;
            queue.push(map[nk]);
          }
